...
export const FigureManagerToolDialog = ({
	isOpen,
	setIsOpen,
}: {
	isOpen: boolean;
	...
	const handleAddCustomFigures = async () => {
		let fileCounter = 1;
		const tempFigures = await Promise.all(
				...
				>
					<div className="fixed inset-0 flex w-screen items-center justify-center p-4 backdrop-blur-[3px] transition-all">
						<DialogPanel className="max-h-[450px] w-full max-w-md overflow-y-auto rounded-xl bg-base-200 p-6 backdrop-blur-2xl">
							<DialogTitle
								as="h3"
								className="sticky top-0 font-medium text-base text-white"
							>
								Subir imagen
							</DialogTitle>
							<Dropzone
							...
							<div className="mt-4 flex flex-row items-center justify-end gap-4">
								<button
									type="button"
									className="btn btn-sm btn-link btn-error text-error decoration-transparent hover:decoration-error"
									onClick={() => {
										setIsOpen(false);
									}}
								>
