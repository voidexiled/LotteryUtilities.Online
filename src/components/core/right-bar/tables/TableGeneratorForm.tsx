...
import { generateRandomTable, resetCandidates } from "@/utils/utils";
import type {
	FiguresContextType,
	ProfileContextType,
	Table,
...
export const TableGeneratorForm = () => {
	...
	const generate = async (): Promise<Table | undefined> => {
		const size = profile.tableOptions.size
			? (profile.tableOptions.size.charAt(0) as unknown as number)
			: 4;
		if (profile.tableOptions.comodin && figures.length < size * size) {
			notifyError("No hay figuras suficientes para generar la tabla.");
			return;
		}
		const { table: newTable } = await generateRandomTable(
			profile,
			figures,
			size,
			1,
	...
	const handleGenerateTable = async () => {
		...
		for (let i = 0; i < tableQuantity; i += batchSize) {
			const batchPromises = Array.from(
				{ length: Math.min(batchSize, tableQuantity - i) },
				(_, j) =>
					generate()
						.then((table) => {
							if (table) {
	...
	return (
		<>
			...
			<div className="flex flex-row gap-2 pt-2 text-xs">
				...
				>
					<span>{generatingTable ? "Generando..." : "Generar Tabla"}</span>
				</button>
				<button
					disabled={generatingTable}
